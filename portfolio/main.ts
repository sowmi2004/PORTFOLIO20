        ...
        <div class="glass-soft px-4 py-4 sm:px-5 sm:py-4">
          ...
          <ul class="mt-3 space-y-2 text-xs sm:text-[0.8rem] text-slate-300 leading-relaxed">
            ${item.achievements
              .map(
                (achievement) =>
                  `<li class="flex gap-2"><span class="mt-[6px] h-[3px] w-[14px] rounded-full bg-accent/70 flex-shrink-0"></span><span>${achievement}</span></li>`,
              )
              .join('')}
          </ul>
        </div>
...
function createFutureVision(): string {
...
function setupSmoothScrolling(root: HTMLElement): void {
  const navButtons = root.querySelectorAll<HTMLButtonElement>('[data-nav-target]');
...
function setupActiveNavHighlight(): void {
  const navButtons = Array.from(
    document.querySelectorAll<HTMLButtonElement>('button[data-nav-target]'),
  );
